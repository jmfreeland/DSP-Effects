...
// Lexicon PCM81-inspired Dual-Plt algorithm for the Polyend Endless: a
// Submixer routing a Plate reverb against a 2-voice "Dual Shifter" FX
// block, in parallel by default - identical shape to Dual-Chmb (see
// dsp/include/dsp/graphs/DualPltAlgorithm.h and
// docs/lexicon-pcm81-dual-chmb.md) with Plate in place of Chamber.
//
// Knob mapping:
...
class PatchImpl : public Patch
{
  public:
    ...
    {
        static_assert(dsp::graphs::DualPltAlgorithm::requiredWorkingBufferSize() <=
                        kWorkingBufferSize,
                      "DualPltAlgorithm needs more working buffer than the Patch provides");
        engine_.prepare(
          kSampleRate,
          std::span<float>(buffer.data(), dsp::graphs::DualPltAlgorithm::requiredWorkingBufferSize()));
    }
    ...
    void handleAction(int actionIdx) override
    {
        switch (static_cast<endless::ActionId>(actionIdx))
        {
            ...
            case endless::ActionId::kLeftFootSwitchHold:
                ...
                engine_.setRouting(seriesRouting_ ? dsp::graphs::DualPltAlgorithm::Routing::kRvbIntoFx
                                                   : dsp::graphs::DualPltAlgorithm::Routing::kParallel);
                break;
        }
    }
    ...
    void init() override
    {
        ...
        engine_.setRouting(dsp::graphs::DualPltAlgorithm::Routing::kParallel);
