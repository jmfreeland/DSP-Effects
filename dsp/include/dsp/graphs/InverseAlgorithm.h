...
{
/**
 * The full PCM81 "Inverse" algorithm: the Inverse reverb Block (see
 * dsp/algorithms/Inverse.h - Duration + independent Low Slope/Mid Slope
 * replacing RT60 decay entirely) wrapped in the same 4-Voice "Reverb
 * Shell" front end as ConcertHallAlgorithm.h - see that file's topology
 * diagram and doc comment, which apply unchanged here (the manual
 * describes this front end as common to every 4-Voice algorithm, not
 * just Concert Hall).
 *
 * This class is presently a near-duplicate of ConcertHallAlgorithm.h,
 * PlateAlgorithm.h, ChamberAlgorithm.h, and InfiniteAlgorithm.h with the
 * reverb Block swapped out - deliberately, rather than templating the
 * Graph on the reverb core type; see PlateAlgorithm.h's doc comment for
 * why. See docs/lexicon-pcm81-inverse.md.
 */
class InverseAlgorithm
{
  public:
    ...
    // Note: no setDecaySeconds()/setLowRatio()/setLink() here, unlike the
    // other three cores - the manual has Duration+Low Slope+Mid Slope
    // fully replace Low Rt/Mid Rt for Inverse, so those aren't exposed as
    // Inverse controls even though the mechanism they'd drive still
    // exists internally (see dsp/algorithms/Inverse.h).
