...
{
/**
 * The full PCM81 "Plate" algorithm: the Plate reverb Block (see
 * dsp/algorithms/Plate.h - Attack + EkoDly/EkoFbk pre-echo on top of the
 * shared ReverbCore) wrapped in the same 4-Voice "Reverb Shell" front end
 * as ConcertHallAlgorithm.h - see that file's topology diagram and doc
 * comment, which apply unchanged here (the manual describes this front
 * end as common to every 4-Voice algorithm, not just Concert Hall).
 *
 * This class is presently a near-duplicate of ConcertHallAlgorithm.h with
 * the reverb Block swapped out and Plate's own Attack/Eko controls added
 * - deliberately, rather than templating the Graph on the reverb core
 * type, until a third 4-Voice algorithm (Chamber/Inverse/Infinite) exists
 * to confirm what's actually common vs. incidental. See
 * docs/lexicon-pcm81-plate.md.
 */
class PlateAlgorithm
{
  public:
    ...
    static constexpr std::size_t requiredWorkingBufferSize()
    {
        ...
        return dsp::algorithms::Plate::requiredWorkingBufferSize() +
    ...
    void prepare(float sampleRate, std::span<float> workingBuffer)
    {
        ...
        std::size_t offset = 0;
        auto reverbSize = dsp::algorithms::Plate::requiredWorkingBufferSize();
    ...
    // -- Plate-specific reverb Block pass-throughs --
