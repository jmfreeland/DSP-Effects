...
{
/**
 * The full PCM81 "Infinite" algorithm: the Infinite reverb Block (see
 * dsp/algorithms/Infinite.h - Chamber's Shape+Spread+pre-echo, plus the
 * freeze mechanism already generic on ReverbCore) wrapped in the same
 * 4-Voice "Reverb Shell" front end as ConcertHallAlgorithm.h - see that
 * file's topology diagram and doc comment, which apply unchanged here
 * (the manual describes this front end as common to every 4-Voice
 * algorithm, not just Concert Hall).
 *
 * This class is presently a near-duplicate of ConcertHallAlgorithm.h,
 * PlateAlgorithm.h, and ChamberAlgorithm.h with the reverb Block swapped
 * out - deliberately, rather than templating the Graph on the reverb
 * core type; see PlateAlgorithm.h's doc comment for why. See
 * docs/lexicon-pcm81-infinite.md.
 */
class InfiniteAlgorithm
{
  public:
    ...
    static constexpr std::size_t requiredWorkingBufferSize()
    {
        ...
        return dsp::algorithms::Infinite::requiredWorkingBufferSize() +
    ...
    void prepare(float sampleRate, std::span<float> workingBuffer)
    {
        ...
        std::size_t offset = 0;
        auto reverbSize = dsp::algorithms::Infinite::requiredWorkingBufferSize();
  ...
  private:
    ...
    dsp::algorithms::Infinite reverb_;
