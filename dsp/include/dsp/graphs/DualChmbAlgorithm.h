...
{
/**
 * The Lexicon PCM81 "Dual-Chmb" algorithm: the first of the manual's five
 * true Dual-FX Pitch algorithms - a Submixer routing a fixed Chamber
 * reverb block against a "Dual Shifter" FX block (two independent
 * PitchShiftVoice-based voices, each with own Delay/Pitch/Level/Pan and
 * Fbk/X-Fbk feedback into both voices' shared input), freely arranged via
 * Sends/Returns/Routing. See dsp/Submixer.h and
 * docs/lexicon-pcm81-reference.md's "The Pitch algorithms" section.
 *
 *   L,R -> Submixer::send() -+-> Rvb block (Chamber, own InLvl/Mix) -----+
 *                              \                                         |
 *                               +-> FX block (Dual Shifter, own InLvl/Mix) +
 *                                                                          |
 *   Routing: Parallel (both independent) / Rvb->FX (series) / FX->Rvb (series, reverse)
 *                                                                          |
 *   Submixer::receive() <-----------------------------------------------+
 *   -> FX Width / Hi-Cut / Adjust / Mix -> output
 *
 * Routing itself isn't inside Submixer (see its own doc comment) - this
 * Graph owns both Block-equivalent sub-effects and sequences them itself,
 * since only it knows which one's output should feed the other's input
 * in series mode.
 */
class DualChmbAlgorithm
{
  public:
