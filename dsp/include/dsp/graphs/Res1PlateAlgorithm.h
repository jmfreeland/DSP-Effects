...
#include "dsp/algorithms/Res1Plate.h"
...
{
/**
 * The full PCM81 "Res1>Plate" algorithm: the Res1Plate Block (six
 * chromatically, directly-tuned resonator voices feeding a fixed Plate
 * reverb in series) wrapped in the Controls row common to every
 * algorithm - In Lvl/Pan conditioning, a Voice Diffusion stage (applied
 * to the material entering the resonators, the same interpretation call
 * used by GlideHallAlgorithm for the "Voice Dif" Controls-row parameter
 * every 6-Voice algorithm lists), and a shared FX Width/Hi-Cut/Adjust
 * chain, finished with a top-level dry/wet Mix.
 *
 *   L,R -> InLvl/InPan -> VoiceDiffusion(L), VoiceDiffusion(R) -> Res1Plate
 *          -> FX Width (StereoRotate) -> Hi-Cut (one-pole) -> FX Adjust
 *   output = lerp(dry, that, mix)
 *
 * See docs/lexicon-pcm81-res1-plate.md for the full source-material
 * mapping and docs/lexicon-pcm81-reference.md for the shared parameter
 * definitions this is built from.
 */
class Res1PlateAlgorithm
{
  public:
    ...
    static constexpr std::size_t requiredWorkingBufferSize()
    {
        ...
        return dsp::algorithms::Res1Plate::requiredWorkingBufferSize() + 2 * voiceDiffuserTotal;
    }
    ...
    void prepare(float sampleRate, std::span<float> workingBuffer)
    {
        ...
        std::size_t offset = 0;
        auto blockSize = dsp::algorithms::Res1Plate::requiredWorkingBufferSize();
    ...
    void setVoicePitch(int index, float hz) { block_.setVoicePitch(index, hz); }
  ...
  private:
    ...
    dsp::algorithms::Res1Plate block_;
