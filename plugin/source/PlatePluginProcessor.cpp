...
LexiconPlateAudioProcessor::LexiconPlateAudioProcessor()
...
float LexiconPlateAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout
LexiconPlateAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(floatParam(kDampingId, "Damping", 0.0f, 1.0f, 0.25f));
    params.push_back(floatParam(kDiffusionId, "Diffusion", 0.0f, 1.0f, 0.85f));
    ...
    params.push_back(floatParam(kPreDelayId, "Pre Delay", 0.0f, 0.93f, 0.01f, "s"));
    params.push_back(
      floatParam(kEarlyReflectionLevelLeftId, "Early Reflections L", 0.0f, 1.0f, 0.25f));
    params.push_back(
      floatParam(kEarlyReflectionLevelRightId, "Early Reflections R", 0.0f, 1.0f, 0.25f));
    params.push_back(
    ...
    // -- Plate-specific: Attack + recirculating EkoDly/EkoFbk pre-echo --
    params.push_back(floatParam(kAttackId, "Attack", 0.0f, 1.0f, 0.6f));
    params.push_back(floatParam(kEkoDelayLeftId, "Eko Delay L", 0.0f, 1.2f, 0.045f, "s"));
    params.push_back(floatParam(kEkoDelayRightId, "Eko Delay R", 0.0f, 1.2f, 0.055f, "s"));
    params.push_back(floatParam(kEkoFeedbackLeftId, "Eko Feedback L", 0.0f, 0.95f, 0.25f));
    params.push_back(floatParam(kEkoFeedbackRightId, "Eko Feedback R", 0.0f, 0.95f, 0.25f));
...
void LexiconPlateAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::PlateAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void LexiconPlateAudioProcessor::releaseResources() {}

bool LexiconPlateAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void LexiconPlateAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
void LexiconPlateAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new LexiconPlateAudioProcessor();
}
