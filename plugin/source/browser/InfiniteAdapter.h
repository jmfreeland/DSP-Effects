...
class InfiniteAdapter : public EngineAdapter
{
  public:
    ...
    // See PlateAdapter.h's importPcm80Preset() for the pattern; Infinite's
    // PCM80 field list is Chamber's (Shape+Spread, not Attack) plus one
    // extra field, Rvb Time Infinite (On/Off), mapped onto this
    // adapter's own freeze - PCM80's "Infinite" field genuinely is the
    // tank-freeze control (see CLAUDE.md's own "freeze already lived on
    // the base" note). Definition, Depth, Chorus have no PCM80 Infinite
    // equivalent and are left untouched.
    const char* pcm80AlgorithmName() const override { return "Infinite"; }
    ...
    void importPcm80Preset(const pcm80::Preset& preset, juce::AudioProcessorValueTreeState& apvts) const override
    {
        ...
        apply("Rvb Time", "Infinite", pid("freeze"), 0.0f, 1.0f, boolean);
        ...
        for (int i = 0; i < 4; ++i)
        {
            apply("Levels", kVoiceLabels[i], voiceParamId("infinite", i, "Level"), -1.0f, 1.0f, dbSigned);
            apply("DelayTime", kVoiceLabels[i], voiceParamId("infinite", i, "Delay"), 0.0f, 1.365f, ms);
            apply("Feedback", kVoiceLabels[i], voiceParamId("infinite", i, "Feedback"), -1.0f, 1.0f,
                  [](const Field& f) { return direct_(f) / 100.0f; });
            apply("Panning", kVoiceLabels[i], voiceParamId("infinite", i, "Pan"), -1.0f, 1.0f, direct);
        }
    }
