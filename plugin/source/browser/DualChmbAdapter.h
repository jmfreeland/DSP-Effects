...
#include "dsp/schema/DualChmbSchema.h"

#include <span>

// Adapts dsp::graphs::DualChmbAlgorithm to EngineAdapter - see
// ConcertHallAdapter.h's doc comment for the pattern; this is
// DualChmbPluginProcessor.cpp's own layout/processBlock, namespaced
// under "dualChmb". Voice pitch defaults use the corrected +-12 cents
// (was +-700, the same wild-detune bug Quad>Hall shipped with - fixed
// at the Graph level in DualChmbAlgorithm.h, mirrored here).
namespace loom::browser
{
namespace dualchmb_detail
{
...
class DualChmbAdapter : public EngineAdapter
{
  public:
    const char* id() const override { return "dualChmb"; }
    const char* displayName() const override { return "Lexicon Dual-Chmb"; }

    std::vector<std::unique_ptr<juce::RangedAudioParameter>> createParameters() const override
    {
        using namespace dualchmb_detail;
        std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
        auto pid = [](const char* suffix) { return prefixedId("dualChmb", suffix); };
        ...
        for (int i = 0; i < 2; ++i)
        {
            params.push_back(floatParam(voiceParamId("dualChmb", i, "Delay"),
                                         "Voice " + juce::String(i + 1) + " Delay", 0.0f, 1.25f,
                                         kDefaultDelay[i], "s", 0.5f));
            params.push_back(floatParam(voiceParamId("dualChmb", i, "Cents"),
                                         "Voice " + juce::String(i + 1) + " Pitch", -3600.0f, 3600.0f,
                                         kDefaultCents[i], "cents"));
            params.push_back(floatParam(voiceParamId("dualChmb", i, "Level"),
                                         "Voice " + juce::String(i + 1) + " Level", 0.0f, 1.0f, 0.7f));
            params.push_back(floatParam(voiceParamId("dualChmb", i, "Pan"),
                                         "Voice " + juce::String(i + 1) + " Pan", -1.0f, 1.0f,
                                         kDefaultPan[i]));
            params.push_back(floatParam(voiceParamId("dualChmb", i, "Feedback"),
                                         "Voice " + juce::String(i + 1) + " Fbk", -1.0f, 1.0f, 0.0f));
            params.push_back(floatParam(voiceParamId("dualChmb", i, "CrossFeedback"),
    ...
    {
        return dsp::schema::dualChmbSchema();
    }

    std::size_t requiredWorkingBufferSize() const override
    {
        return dsp::graphs::DualChmbAlgorithm::requiredWorkingBufferSize();
    }
    ...
    {
        auto v = [&](const char* suffix) { return paramValue(apvts, prefixedId("dualChmb", suffix)); };
        ...
        engine_.setRouting(
          static_cast<dsp::graphs::DualChmbAlgorithm::Routing>(static_cast<int>(v("routing"))));
        ...
        for (int i = 0; i < 2; ++i)
        {
            auto vv = [&](const char* suffix) {
                return paramValue(apvts, voiceParamId("dualChmb", i, suffix));
            };
