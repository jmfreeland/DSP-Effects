...
// InversePluginProcessor.cpp's own layout/processBlock, namespaced
// under "inverse". Unlike the other three cores, Inverse has no
// Decay/Low Ratio/Link - Duration + Low Slope/Mid Slope replace them.
namespace loom::browser
{
class InverseAdapter : public EngineAdapter
{
  public:
    const char* id() const override { return "inverse"; }
    const char* displayName() const override { return "Lexicon Inverse"; }
    ...
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> createParameters() const override
    {
        ...
        params.push_back(floatParam(pid("duration"), "Duration", 0.05f, 8.0f, 1.0f, "s", 0.5f));
        ...
        for (int i = 0; i < 4; ++i)
        {
            params.push_back(floatParam(voiceParamId("inverse", i, "Delay"),
                                         "Voice " + juce::String(i + 1) + " Delay", 0.0f, 1.365f,
                                         kDefaultDelay[i], "s"));
            params.push_back(floatParam(voiceParamId("inverse", i, "Feedback"),
                                         "Voice " + juce::String(i + 1) + " Feedback", -1.0f, 1.0f,
                                         kDefaultFeedback[i]));
            params.push_back(floatParam(voiceParamId("inverse", i, "Level"),
                                         "Voice " + juce::String(i + 1) + " Level", -1.0f, 1.0f,
                                         kDefaultLevel[i]));
            params.push_back(floatParam(voiceParamId("inverse", i, "Pan"),
    ...
    {
        return dsp::schema::inverseAlgorithmSchema();
    }

    std::size_t requiredWorkingBufferSize() const override
    {
        return dsp::graphs::InverseAlgorithm::requiredWorkingBufferSize();
    }
    ...
    {
        auto v = [&](const char* suffix) { return paramValue(apvts, prefixedId("inverse", suffix)); };
        ...
        for (int i = 0; i < 4; ++i)
        {
            auto vv = [&](const char* suffix) {
                return paramValue(apvts, voiceParamId("inverse", i, suffix));
            };
