...
// Adapts dsp::graphs::PlateAlgorithm to EngineAdapter - see
// ConcertHallAdapter.h's doc comment for the pattern; this is
// PlatePluginProcessor.cpp's own layout/processBlock, namespaced under
// "plate".
namespace loom::browser
{
class PlateAdapter : public EngineAdapter
{
  public:
    const char* id() const override { return "plate"; }
    const char* displayName() const override { return "Lexicon Plate"; }

    std::vector<std::unique_ptr<juce::RangedAudioParameter>> createParameters() const override
    {
        std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
        auto pid = [](const char* suffix) { return prefixedId("plate", suffix); };
        ...
        params.push_back(floatParam(pid("damping"), "Damping", 0.0f, 1.0f, 0.25f));
        params.push_back(floatParam(pid("diffusion"), "Diffusion", 0.0f, 1.0f, 0.85f));
        ...
        params.push_back(floatParam(pid("earlyReflectionLevelLeft"), "Early Reflections L", 0.0f, 1.0f, 0.25f));
        params.push_back(floatParam(pid("earlyReflectionLevelRight"), "Early Reflections R", 0.0f, 1.0f, 0.25f));
        params.push_back(
        ...
        params.push_back(floatParam(pid("attack"), "Attack", 0.0f, 1.0f, 0.6f));
        params.push_back(floatParam(pid("ekoDelayLeft"), "Eko Delay L", 0.0f, 1.2f, 0.045f, "s"));
        params.push_back(floatParam(pid("ekoDelayRight"), "Eko Delay R", 0.0f, 1.2f, 0.055f, "s"));
        params.push_back(floatParam(pid("ekoFeedbackLeft"), "Eko Feedback L", 0.0f, 0.95f, 0.25f));
        params.push_back(floatParam(pid("ekoFeedbackRight"), "Eko Feedback R", 0.0f, 0.95f, 0.25f));
        ...
        for (int i = 0; i < 4; ++i)
        {
            params.push_back(floatParam(voiceParamId("plate", i, "Delay"),
                                         "Voice " + juce::String(i + 1) + " Delay", 0.0f, 1.365f,
                                         kDefaultDelay[i], "s"));
            params.push_back(floatParam(voiceParamId("plate", i, "Feedback"),
                                         "Voice " + juce::String(i + 1) + " Feedback", -1.0f, 1.0f,
                                         kDefaultFeedback[i]));
            params.push_back(floatParam(voiceParamId("plate", i, "Level"),
                                         "Voice " + juce::String(i + 1) + " Level", -1.0f, 1.0f,
                                         kDefaultLevel[i]));
            params.push_back(floatParam(voiceParamId("plate", i, "Pan"),
    ...
    {
        return dsp::schema::plateAlgorithmSchema();
    }

    std::size_t requiredWorkingBufferSize() const override
    {
        return dsp::graphs::PlateAlgorithm::requiredWorkingBufferSize();
    }
    ...
    {
        auto v = [&](const char* suffix) { return paramValue(apvts, prefixedId("plate", suffix)); };
        ...
        for (int i = 0; i < 4; ++i)
        {
            auto vv = [&](const char* suffix) { return paramValue(apvts, voiceParamId("plate", i, suffix)); };
    ...
    // Maps a decoded PCM80 "Plate" preset (tools/pcm80-import) onto this
    // adapter's own parameters. PCM80's Plate algorithm shares its name
    // and general topology with this Block (see ReverbCore.h's own doc
    ...
    void importPcm80Preset(const pcm80::Preset& preset, juce::AudioProcessorValueTreeState& apvts) const override
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            apply("Levels", kVoiceLabels[i], voiceParamId("plate", i, "Level"), -1.0f, 1.0f, dbSigned);
            apply("DelayTime", kVoiceLabels[i], voiceParamId("plate", i, "Delay"), 0.0f, 1.365f, ms);
            apply("Feedback", kVoiceLabels[i], voiceParamId("plate", i, "Feedback"), -1.0f, 1.0f,
                  [](const Field& f) { return direct_(f) / 100.0f; });
            apply("Panning", kVoiceLabels[i], voiceParamId("plate", i, "Pan"), -1.0f, 1.0f, direct);
        }
    }
