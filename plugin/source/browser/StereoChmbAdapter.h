...
// Adapts dsp::graphs::StereoChmbAlgorithm to EngineAdapter - see
// ConcertHallAdapter.h's doc comment for the pattern; this is
// StereoChmbPluginProcessor.cpp's own layout/processBlock, namespaced
// under "stereoChmb". Shift default corrected to +-12 cents (was 700 -
// the same wild-detune bug as Quad>Hall/the Dual family, fixed at the
// Graph level in StereoChmbAlgorithm.h).
namespace loom::browser
{
namespace stereochmb_detail
{
...
class StereoChmbAdapter : public EngineAdapter
{
  public:
    const char* id() const override { return "stereoChmb"; }
    const char* displayName() const override { return "Lexicon Stereo-Chmb"; }

    std::vector<std::unique_ptr<juce::RangedAudioParameter>> createParameters() const override
    {
        using namespace stereochmb_detail;
        std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
        auto pid = [](const char* suffix) { return prefixedId("stereoChmb", suffix); };
        ...
        params.push_back(floatParam(pid("shiftCents"), "Shift", -3600.0f, 3600.0f, 12.0f, "cents"));
    ...
    {
        return dsp::schema::stereoChmbSchema();
    }

    std::size_t requiredWorkingBufferSize() const override
    {
        return dsp::graphs::StereoChmbAlgorithm::requiredWorkingBufferSize();
    }
    ...
    {
        auto v = [&](const char* suffix) { return paramValue(apvts, prefixedId("stereoChmb", suffix)); };
        ...
        engine_.setRouting(
          static_cast<dsp::graphs::StereoChmbAlgorithm::Routing>(static_cast<int>(v("routing"))));
