...
LexiconInfiniteAudioProcessor::LexiconInfiniteAudioProcessor()
...
float LexiconInfiniteAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout
LexiconInfiniteAudioProcessor::createParameterLayout()
{
    ...
    // -- Reverb core --
    params.push_back(floatParam(kDecayId, "Decay", 0.3f, 8.0f, 6.0f, "s", 0.5f));
    ...
    // -- Shape+Spread onset envelope + recirculating EkoDly/EkoFbk pre-echo (inherited from Chamber) --
    params.push_back(floatParam(kShapeId, "Shape", 0.0f, 1.0f, 0.2f));
    params.push_back(floatParam(kSpreadId, "Spread", 0.0f, 1.0f, 0.6f));
    params.push_back(floatParam(kEkoDelayLeftId, "Eko Delay L", 0.0f, 1.2f, 0.06f, "s"));
    params.push_back(floatParam(kEkoDelayRightId, "Eko Delay R", 0.0f, 1.2f, 0.07f, "s"));
    params.push_back(floatParam(kEkoFeedbackLeftId, "Eko Feedback L", 0.0f, 0.95f, 0.2f));
    params.push_back(floatParam(kEkoFeedbackRightId, "Eko Feedback R", 0.0f, 0.95f, 0.2f));
...
void LexiconInfiniteAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::InfiniteAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void LexiconInfiniteAudioProcessor::releaseResources() {}

bool LexiconInfiniteAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void LexiconInfiniteAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
void LexiconInfiniteAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new LexiconInfiniteAudioProcessor();
}
