#include "ReverseShiftPluginProcessor.h"
...
constexpr auto kLeftLengthId = "leftLength";
constexpr auto kRightLengthId = "rightLength";
...
EventideReverseShiftAudioProcessor::EventideReverseShiftAudioProcessor()
...
float EventideReverseShiftAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout
EventideReverseShiftAudioProcessor::createParameterLayout()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;

    params.push_back(floatParam(kGrainId, "Grain", 0.01f, 0.3f, 0.07f, "s"));
    params.push_back(floatParam(kLeftLengthId, "Left Length", 0.02f, 1.4f, 0.15f, "s"));
    params.push_back(floatParam(kRightLengthId, "Right Length", 0.02f, 1.4f, 0.15f, "s"));
...
void EventideReverseShiftAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::ReverseShiftAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void EventideReverseShiftAudioProcessor::releaseResources() {}

bool EventideReverseShiftAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void EventideReverseShiftAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    engine_.setGrainSeconds(paramValue(kGrainId));
    engine_.setLeftLengthSeconds(paramValue(kLeftLengthId));
    engine_.setRightLengthSeconds(paramValue(kRightLengthId));
...
juce::AudioProcessorEditor* EventideReverseShiftAudioProcessor::createEditor()
{
    return new LoomPluginEditor(*this, dsp::schema::reverseShiftSchema());
}

void EventideReverseShiftAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
...
void EventideReverseShiftAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new EventideReverseShiftAudioProcessor();
}
