#include "DualPltPluginProcessor.h"
...
DualPltAudioProcessor::DualPltAudioProcessor()
...
float DualPltAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout DualPltAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(floatParam(kAttackId, "Attack", 0.0f, 1.0f, 0.5f));
...
void DualPltAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::DualPltAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void DualPltAudioProcessor::releaseResources() {}

bool DualPltAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void DualPltAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    engine_.setRouting(
      static_cast<dsp::graphs::DualPltAlgorithm::Routing>(static_cast<int>(paramValue(kRoutingId))));
...
juce::AudioProcessorEditor* DualPltAudioProcessor::createEditor()
{
    return new LoomPluginEditor(*this, dsp::schema::dualPltSchema());
}

void DualPltAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
...
void DualPltAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new DualPltAudioProcessor();
}
