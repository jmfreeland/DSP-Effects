...
LexiconChamberAudioProcessor::LexiconChamberAudioProcessor()
...
float LexiconChamberAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout
LexiconChamberAudioProcessor::createParameterLayout()
{
    ...
    // -- Reverb core --
    params.push_back(floatParam(kDecayId, "Decay", 0.3f, 8.0f, 2.8f, "s", 0.5f));
    ...
    // -- Chamber-specific: Shape+Spread onset envelope + recirculating EkoDly/EkoFbk pre-echo --
...
void LexiconChamberAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::ChamberAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void LexiconChamberAudioProcessor::releaseResources() {}

bool LexiconChamberAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void LexiconChamberAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
void LexiconChamberAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new LexiconChamberAudioProcessor();
}
