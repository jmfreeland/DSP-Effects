#include "DualChmbPluginProcessor.h"
...
DualChmbAudioProcessor::DualChmbAudioProcessor()
...
float DualChmbAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout DualChmbAudioProcessor::createParameterLayout()
{
...
void DualChmbAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::DualChmbAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void DualChmbAudioProcessor::releaseResources() {}

bool DualChmbAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void DualChmbAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    engine_.setRouting(
      static_cast<dsp::graphs::DualChmbAlgorithm::Routing>(static_cast<int>(paramValue(kRoutingId))));
...
juce::AudioProcessorEditor* DualChmbAudioProcessor::createEditor()
{
    return new LoomPluginEditor(*this, dsp::schema::dualChmbSchema());
}

void DualChmbAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
...
void DualChmbAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new DualChmbAudioProcessor();
}
