#include "StereoChmbPluginProcessor.h"

#include "LoomPluginEditor.h"
#include "dsp/schema/StereoChmbSchema.h"
...
StereoChmbAudioProcessor::StereoChmbAudioProcessor()
...
float StereoChmbAudioProcessor::paramValue(const char* id) const
{
    return apvts.getRawParameterValue(id)->load();
}

juce::AudioProcessorValueTreeState::ParameterLayout StereoChmbAudioProcessor::createParameterLayout()
{
...
void StereoChmbAudioProcessor::prepareToPlay(double sampleRate, int /* samplesPerBlock */)
{
    workingBuffer_.assign(dsp::graphs::StereoChmbAlgorithm::requiredWorkingBufferSize(), 0.0f);
    engine_.prepare(static_cast<float>(sampleRate), workingBuffer_);
}

void StereoChmbAudioProcessor::releaseResources() {}

bool StereoChmbAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    return layouts.getMainInputChannelSet() == juce::AudioChannelSet::stereo() &&
           layouts.getMainOutputChannelSet() == juce::AudioChannelSet::stereo();
}

void StereoChmbAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    engine_.setRouting(
      static_cast<dsp::graphs::StereoChmbAlgorithm::Routing>(static_cast<int>(paramValue(kRoutingId))));
...
juce::AudioProcessorEditor* StereoChmbAudioProcessor::createEditor()
{
    return new LoomPluginEditor(*this, dsp::schema::stereoChmbSchema());
}

void StereoChmbAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
...
void StereoChmbAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new StereoChmbAudioProcessor();
}
