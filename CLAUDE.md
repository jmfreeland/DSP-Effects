...
- **Block** (`dsp/include/dsp/algorithms/*.h`) — a reverb/effect core built from Primitives/Components (e.g. `ConcertHall.h`), independent of any SDK. Exposes `prepare(sampleRate, workingBuffer)`, per-parameter setters, `process(left, right)` (whole-block) and/or `processSample(float&, float&)` (single-sample step, so a Graph can interleave it with other per-sample Components), and `reset()`.
