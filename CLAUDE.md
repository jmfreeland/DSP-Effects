...
- **Block** (`dsp/include/dsp/algorithms/*.h`) — a reverb/effect core built from Primitives/Components, independent of any SDK. Exposes `prepare(sampleRate, workingBuffer)`, per-parameter setters, `process(left, right)` (whole-block) and/or `processSample(float&, float&)` (single-sample step, so a Graph can interleave it with other per-sample Components), and `reset()`. The PCM81's five reverb cores (Concert Hall, Plate, Chamber, Inverse, Infinite) share one signal path per the manual, so `ReverbCore.h` implements that shared path once (Diffusion/PreDelay/8-line Householder FDN tank/early reflections/Definition/Depth/Rvb In-Out/freeze) and each concrete core derives from it, adding only its own documented control(s) via three protected hooks (`applyPreEcho()` for Plate/Chamber/Infinite's recirculating EkoDly/EkoFbk pre-echo, `effectiveDiffusion()` for dynamic diffusion shaping like Plate's Attack, `outputEnvelope()` for a wet-output gain envelope like Chamber's Shape+Spread). `ConcertHall.h` is the reference core (no additions); `Plate.h` adds Attack + pre-echo; `Chamber.h` adds Shape+Spread + pre-echo.
